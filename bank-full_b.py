...
data.poutcome.replace(['unknown', 'other', 'failure', 'success'], [0,1,2,3], inplace = True)
data.marital.replace(['married', 'divorced', 'single'], [0,1,2], inplace = True)
data.drop(['job', 'education','contact','day','month' ], axis = 1, inplace = True)

x = np.array(data.drop(['y'], 1))
y = np.array(data.y)  # 0 No se suscribe, 1 Si se suscribe
...
tabla_comparativa = pd.DataFrame(modelos, columns = ['Acurracy de Validacion', 'Acurracy de Test', 'AUC', 'Presicion','Recall', 'f1-score'], index =['LogisticRegression', 'DecissionTreeClassifier', 'KNheighorsClassifier', 'GaussianNB', 'RandomForestClassifier'])
